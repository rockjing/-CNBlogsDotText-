...
namespace Dottext.Framework.Syndication
{
	...
	public abstract class BaseRssWriter : BaseSyndicationWriter
	{
		...
		protected virtual void WalkEntries()
		{
		...
		{
			if(config is Configuration.SiteBlogConfig)
			{
				string BlogUrl=Dottext.Framework.Util.Globals.GetBlogAppUrl(entry.Link,System.Web.HttpContext.Current.Request.ApplicationPath);
				uformat=new UrlFormats(BlogUrl);
