...
namespace Dottext.Common.Data
{
	...
	public class Cacher
	{
		...
		public static LinkCategoryCollection GetActiveCategories(CacheTime ct, HttpContext context)
		{
			...
			if(categories == null)
			{
				categories = Links.GetCategoriesWithLinks(true);//.GetActiveCategories();
				if(categories != null)
				{
		...
		public static EntryCollection GetMonth(DateTime dt, CacheTime ct, HttpContext context)
		{
			...
			//context.Response.Write(context.Request.RawUrl+dt.ToString()+"<br>"+key+"<br>");
			...
			if(month == null)
			{
				...
				month =  Entries.GetEntryCollection(query);
		...
		public static EntryCollection GetMonth(DateTime dt, CacheTime ct, HttpContext context,PostType postType)
		{
			string key = string.Format(EntryMonthKey,dt.ToString("yyyyMM"),BlogID(context))+"PostType"+((int)postType).ToString();
			//Dottext.Framework.Logger.LogManager.Log("GetMonth",key);
			//context.Response.Write(context.Request.RawUrl+dt.ToString()+"<br>"+key+"<br>");
			EntryCollection month = (EntryCollection)context.Cache[key];
			if(month == null)
			{
				EntryQuery query = new EntryQuery(PostConfig.IsActive,postType);
				query.StartDate = dt;
				query.EndDate = dt.AddMonths(1);
				month =  Entries.GetEntryCollection(query);
				//month = Entries.GetPostCollectionByMonth(dt.Month,dt.Year);
		...
		{
			return Dottext.Framework.Configuration.Config.CurrentBlog(context).BlogID;
		}
		...
		public static EntryCollection GetEntriesByCategory(int count, CacheTime ct, HttpContext context, int categoryID)
		{
			...
			if(ec == null)
			{
				...
				query.PostConfig =  PostConfig.IsActive;
				query.ItemCount = count;
				query.CategoryID = categoryID;
				ec = Entries.GetEntryCollection(query);
		...
		public static EntryCollection GetEntriesByCategory(int count, CacheTime ct, HttpContext context, string CategoryName)
		{
			...
			if(ec == null)
			{
				...
				query.PostConfig =  PostConfig.IsActive;
				query.ItemCount = count;
				query.CategoryTitle = CategoryName;
				ec = Entries.GetEntryCollection(query);
		...
		public static Entry SingleEntry(string EntryName, CacheTime ct, HttpContext context)
		{
			...
			if(entry == null)
			{
				entry = Entries.GetEntry(EntryName,PostConfig.IsActive);	
				if(entry != null)
				{
		...
		public static Entry SingleEntry(int EntryID, CacheTime ct, HttpContext context)
		{
			...
			if(entry == null)
			{
				if(Security.IsAdmin)
				{
					entry = Entries.GetEntry(EntryID);
				}
				else
				{
					entry = Entries.GetEntry(EntryID,PostConfig.IsActive);		
				}
				if(entry != null)
				{
					CacherCache(key,context,entry,ct);
