...
namespace Dottext.Web.Admin.Pages
{
	public class EditGalleries : AdminPage
	{
		...
		private  bool ValidateFileType(string filename)
		{
			...
			if(!isOK)
			{
				Messages.ShowError(string.Format("上传图像类型不符合要求,只能上传{0}类型的图像文件",regexstr.Substring(0,regexstr.Length-1)));
			}
			return isOK;
		}
		...
		private  bool ValidateFileSize(int size)
		{
			...
			if(size>limit*1024)
			{
				Messages.ShowError(string.Format("上传文件超出规定大小{0}KB",limit.ToString()));
				return false;
			}
			return true;
		}
