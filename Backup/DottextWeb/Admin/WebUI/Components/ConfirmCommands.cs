...
namespace Dottext.Web.Admin
{
	...
	public abstract class ConfirmCommand
	{
		...
		protected string _tabSectionID;
		...
		public virtual string TabSectionID
		{
	...
	public abstract class DeleteTargetCommand : ConfirmCommand
	{
		...
		public override string PromptMessage
		{
			get 
			{
				if(_targetTitle==null||_targetTitle=="")
				{
					Entry entry=Entries.GetEntry(_targetID);
					if(entry!=null)
					{
						_targetTitle=entry.Title;
					}
					else
					{
						_targetTitle=_targetID.ToString();
					}
				}
				else
				{
				...
				return FormatMessage(base.PromptMessage, _targetName,_targetTitle); 
			}
			set { _promptMessage = value; }
		}
	...
	public class DeletePostCommand : DeleteTargetCommand
	{
		...
		/*public DeletePostCommand(int postID,string postTitle):this(postID)
		{
			_targetTitle=postTitle;
		}*/
		...
		public override string Execute()
		{
			try
			{
				...
				return FormatMessage(ExecuteSuccessMessage, _targetName, _targetTitle);
			}
			catch (Exception ex)
			{
				return FormatMessage(ExecuteFailureMessage, _targetName, _targetTitle, ex.Message);
			}
		}
	}
	#endregion
	...
	public class DeleteCategoryCommand : DeleteTitledTargetCommand
	{
		...
		public DeleteCategoryCommand(int categoryID, string categoryTitle,int BlogID)
		{	
			_targetName = "Category";
			_targetID = categoryID;
			_itemTitle = categoryTitle;
		...
		public override string Execute()
		{
			try
			{
				if(this._BlogID==-999999)
				{
					Links.DeleteLinkCategory(_targetID);
				}
				else
				{
					Links.DeleteLinkCategory(_targetID,_BlogID);
				}
				if(_BlogID==-1)
				{
					Dottext.Framework.Configuration.Config.RemoveSiteBlogConfigByCategoryID(_targetID);
				}
		...
		public override string Execute()
		{
			try
			{
				...
				if (imageList.Count > 0)
				{
					...
					foreach (Dottext.Framework.Components.Image currentImage in imageList)
					{
	...
	{		
		...
		public override string Execute()
		{
			try
			{
				Dottext.Framework.Components.Image currentImage = Images.GetSingleImage(_targetID, false);
