namespace Dottext.Web.AggSite
{
	...
	public class Search : System.Web.UI.UserControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				((AggSite.SearchForm)con).SearchUrl=string.Format("{0}BlogSearch.aspx?q=",Config.CurrentBlog().FullyQualifiedUrl);
			}
			//UI.Globals.RegisterMyScript(this.Page,"Search.js");
			string url=UI.UIText.SiteUrl;
			if(!url.EndsWith("/"))
			{
				url+="/";
			}
			...
			if(!IsPostBack)
			{
				...
				string searchText = Server.UrlDecode(Request.QueryString["q"]);
