...
<%@ OutputCache Duration="30" VaryByParam="none" VaryByCustom="BlogID"%>
...
<asp:repeater id="RecentPostsRepeater" runat="server">
	...
	<ItemTemplate>
		<li class="NavLinkli">
			...
			(<asp:Literal runat = "server" Text = '<%# Dottext.Web.UI.Globals.RemoveHtml(DataBinder.Eval(((RepeaterItem)Container).DataItem,"Author").ToString()) %>' ID="Literal1"/> <asp:Literal runat = "server" Text = '<%# (DateTime.Parse(DataBinder.Eval(((RepeaterItem)Container).DataItem,"DateCreated").ToString())).ToString("MM-dd HH:mm") %>' ID="Literal2"/>
			фюбш:<asp:Literal runat = "server" Text = '<%# DataBinder.Eval(((RepeaterItem)Container).DataItem,"FeedBackCount") %>' ID="Literal3"/> тд╤а:<asp:Literal runat = "server" Text = '<%# DataBinder.Eval(((RepeaterItem)Container).DataItem,"ViewCount") %>' ID="Literal4"/>)
		</li>
	</ItemTemplate>
	<FooterTemplate>
		</ul>
