	...
	public class BlogExistingPageHandler :  IHttpHandlerFactory
	{
		...
		public virtual IHttpHandler GetHandler(HttpContext context, string requestType, string url, string path)
		{
			...
			if(ConfigProvider.Instance().IsAggregateSite)
			{
				...
				url = Regex.Replace(url,app,"/",RegexOptions.IgnoreCase);//url.ToLower().Remove(url.ToLower().LastIndexOf(app),(app.Length) + 1);
				app = "\\\\"+config.CleanApplication+"\\\\";
				path = Regex.Replace(path,app,"/",RegexOptions.IgnoreCase);//path.ToLower().Remove(path.ToLower().LastIndexOf(app),(app.Length + 1));
