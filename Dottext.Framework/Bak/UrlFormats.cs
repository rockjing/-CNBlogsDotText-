...
namespace Dottext.Framework.Format
{
	...
	public class UrlFormats
	{
		...
		{
			return GetUrl("favorite/{0}.aspx",categoryID);
		}
		...
		{
			return GetUrl("Messages/" + entry.DateCreated.ToString("yyyy'/'MM'/'dd") + "/{0}.aspx", entry.HasEntryName ? entry.EntryName : entry.EntryID.ToString());
		}
		...
		{
			return GetUrl("{0}/{1}.aspx",path,dt.ToString("yyyy'/'MM"));
		}
