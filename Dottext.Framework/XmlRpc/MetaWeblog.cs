...
namespace Dottext.Framework.XmlRpc
{
	/// <summary>
	/// Summary description for MetaWeblog.
	/// </summary>
	public class MetaWeblog : XmlRpcService, Dottext.Framework.XmlRpc.IMetaWeblog
	{
		public MetaWeblog()
		{
		...
		public bool editPost(string postid,	string username,string password,Post post,bool publish)
		{
			...
			if(ValidateUser(username,password,config.AllowServiceAccess))
			{
				CategoryEntry entry = Entries.GetCategoryEntry(Int32.Parse(postid),PostConfig.Empty);
				if(entry != null)
				{
		...
		public Post[] getRecentPosts(string blogid,string username,string password,int numberOfPosts)
		{
			if(ValidateUser(username,password,Config.CurrentBlog().AllowServiceAccess))
			{
				...
				query.ItemCount = numberOfPosts;
				query.PostType = PostType.BlogPost;
				query.PostConfig = PostConfig.Empty;

				CategoryEntryCollection ec = Entries.GetCategoryEntryCollection(query);
		...
		public CategoryInfo[] getCategories(string blogid,string username,string password)
		{
			...
			if(ValidateUser(username,password,config.AllowServiceAccess))
			{
				LinkCategoryCollection lcc = Links.GetCategoriesByType(CategoryType.PostCollection,false);
				if(lcc == null)
				{
		...
		public string newPost(string blogid, string username, string password, Post post, bool publish)
		{
			...
			if(ValidateUser(username,password,config.AllowServiceAccess))
			{
				...
				//post.categories[0]="92";
				/*if(post.categories!=null&&post.categories[0]!=""&&post.categories[0]!=null)
				{
					entry.Categories = post.categories;
				}
				else
		...
		/*[XmlRpcMethod("blogger.newPost",
			 ...
			 + "blogger API. Returns postid as a string.")]
		...
		public MtCategory[] GetPostCategories(string postid, string username, string password)
		{
			if(ValidateUser(username,password,Config.CurrentBlog().AllowServiceAccess))
			{
				...
				if (postCategories.Count > 0)
				{
					...
					LinkCategoryCollection cats = Links.GetCategoriesByType(CategoryType.PostCollection, false);
